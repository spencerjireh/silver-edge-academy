...
// Get API base URL from environment variable, with fallback for relative paths
const getApiBase = (): string => {
  const envUrl = import.meta.env.VITE_API_URL
  if (envUrl) {
    // If env URL ends with /api, append /student
    // If env URL is just the backend URL (no /api), append /api/student
    if (envUrl.endsWith('/api')) {
      return `${envUrl}/student`
    }
    return `${envUrl}/api/student`
  }
  // Fallback for relative paths (when running through a proxy)
  return '/api/student'
}
